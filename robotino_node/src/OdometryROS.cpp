...
void OdometryROS::setTfPrefix(std::string &prefix){// ITMO University
	tf_prefix = prefix;
}
		...
		float vx, float vy, float omega, unsigned int sequence )
{
	...
	odometry_msg_.header.frame_id = (tf_prefix == "no_prefix") ? "odom" : (tf_prefix + "/odom"); // ITMO University. Was only {= "odom"}
	odometry_msg_.header.stamp = stamp_;
	odometry_msg_.child_frame_id = (tf_prefix == "no_prefix") ? "base_link" : (tf_prefix + "/base_link"); // ITMO University. Was only {= "base_link"}
	...
	odometry_transform_.header.frame_id = (tf_prefix == "no_prefix") ? "odom" : (tf_prefix + "/odom"); // ITMO University. Was only {= "odom"}
	odometry_transform_.header.stamp = odometry_msg_.header.stamp;
	odometry_transform_.child_frame_id = (tf_prefix == "no_prefix") ? "base_link" : (tf_prefix + "/base_link"); // ITMO University. Was only {= "base_link"}
