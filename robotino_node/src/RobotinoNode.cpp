	...
	: nh_("~")
{
	...
	distances_clearing_pub_ = nh_no_private.advertise<sensor_msgs::PointCloud>("distance_sensors_clearing", 1, true);
...
void RobotinoNode::initModules()
{
	...
	//**This was written by CSI Department (ITMO University) {	
	...
	distance_sensor_array_.setTfPrefix( tf_prefix );
	//** }
...
{	
	...
	//** }
	//**This was written in CSI Department (ITMO University) {
	distances_clearing_msg_.header.frame_id = (tf_prefix == "no_prefix") ? "base_link" : (tf_prefix + "/base_link") ;	
	//** }
	distances_clearing_msg_.header.stamp = curr_time_;
	distances_clearing_msg_.points.resize( 1);//720 ); //ITMO
...
void RobotinoNode::publishJointStateMsg()
{
	...
	//16:1 -- reduction ratio; 2048 increments per revolution -- ecncoder's resolution 
	joint_state_msg_.position[0] = ( motor_positions_[2] / 16.0 / 2048.0 ) * (2 * 3.142); //( motor_positions_[2] / 16 ) * (2 * 3.142);  //ITMO
	joint_state_msg_.position[1] = ( motor_positions_[0] / 16.0 / 2048.0 ) * (2 * 3.142); //( motor_positions_[0] / 16 ) * (2 * 3.142);  //ITMO
	joint_state_msg_.position[2] = ( motor_positions_[1] / 16.0 / 2048.0 ) * (2 * 3.142); //( motor_positions_[1] / 16 ) * (2 * 3.142);  //ITMO
